...
public class TvShowAdapter extends RecyclerView.Adapter<TvShowAdapter.GridViewHolder> {
    ...
    public TvShowAdapter(Context context, Integer movieLoaded, String[] title, String[] id, double[] voting, String[] imgPath, String[] date) {
    ...
    private void saveFav(@NonNull Integer indx) {
        ...
        values.put( TvProvider.KEY_MID, id[indx] );
        values.put( TvProvider.KEY_TITLE, title[indx] );
        values.put( TvProvider.KEY_PATH, imgPath[indx] );
        values.put( TvProvider.KEY_DATE, date[indx] );
        values.put( TvProvider.KEY_VOTE, voting[indx] );

        context.getContentResolver().insert( TvProvider.CONTENT_URI, values );
